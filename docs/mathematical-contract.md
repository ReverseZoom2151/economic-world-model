...
|---|---|---|
| Scalar fixed-point multiplicity and stability | `ewm.equilibrium`, `ewm.scenarios.scalar` | Analytical root count plus package-import-free direct equation and bracketing in `tests/integration/papers/test_independent_numerical_oracles.py` |
...
| L3 to L6 substrate boundaries | `ewm.capabilities`, `ewm.experiments.claims` | Sixteen blocked readiness artifacts plus adversarial evidence gates in `tests/conformance/test_han_l3_l6_readiness.py` and `tests/unit/capabilities/test_capability_levels.py` |
| Constraint-preserving transitions | `ewm.core.reconciliation`, `ewm.core.world` | Induction and atomic-failure checks in `tests/unit/core/assurance/test_reconciliation.py` |
| Paper source locks and observed local verification | `references/papers.toml`, `ewm.experiments.source_verification`, `ewm.conformance`, `scripts/verify_sources.py`, `scripts/run_conformance.py` | Registry, mismatch, absence, and conformance-report checks in `tests/unit/experiments/test_source_verification.py`, `tests/integration/papers/test_paper_traceability.py`, and `tests/integration/papers/test_conformance_source_verification.py` |
| Audited replication targets | `references/replication-targets.toml` | Classification, symbol, evidence-path, and exact-claim coverage checks in `tests/integration/papers/test_replication_targets.py` |
...
| Sealed artifact verification and FX replay | `ewm.experiments.verification`, `ewm.experiments.replay`, `ewm.core.replay` | Tamper, collision, installed-wheel, and deterministic replay tests in `tests/integration/experiments/test_artifact_integrity.py`, `tests/integration/experiments/test_run_cli.py`, and `tests/integration/experiments/test_run_replay.py` |
